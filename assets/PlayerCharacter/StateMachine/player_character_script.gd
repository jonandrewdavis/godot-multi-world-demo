...
@onready var jump_particles = preload("res://assets/PlayerCharacter/Vfx/jump_particles.tscn")
@onready var land_particles = preload("res://assets/PlayerCharacter/Vfx/land_particles.tscn")
...
func _ready():
	...
	tree_exiting.connect(player_cleanup)

	# INFO: Disable input and physics processing if we aren't the authority
	# NOTE: "godot_plush_skin" will still broadcast the animation state via rpc from the authority
	if not is_multiplayer_authority(): 
		...
		godot_plush_skin.set_physics_process(false)
		cam_holder.active = false
		return

	cam_holder.active = true
...
func _process(delta: float):
	...
	# TODO: can use 1, 2, 3, just minus 1 to get the world index
	if Input.is_action_just_pressed("debug1"):
		if main.current_world != 0:
			sync.public_visibility = false # CRITICAL: Helps prevent an error caused by sending data
			main.request_move_to_world.rpc_id(1, 0)
	elif Input.is_action_just_pressed("debug2"):
		if main.current_world != 1:
			sync.public_visibility = false # CRITICAL: Helps prevent an error caused by sending data
			main.request_move_to_world.rpc_id(1, 1)
	elif Input.is_action_just_pressed("menu"):
		if Input.mouse_mode == Input.MOUSE_MODE_CAPTURED:
			sync.public_visibility = true # CRITICAL: Helps prevent an error caused by sending data
			main.menu.hide() 
...
func _filter_peer_ids(id: int):
	...
	if id == 1:
		return main.server_worlds_enabled

	return main.current_players.has(id) and main.current_players[id].world_id == main.current_world		

func player_cleanup():
	sync.public_visibility = false # CRITICAL: Helps prevent error when leaving the game (n
	sync.set_visibility_update_mode(MultiplayerSynchronizer.VISIBILITY_PROCESS_NONE)
